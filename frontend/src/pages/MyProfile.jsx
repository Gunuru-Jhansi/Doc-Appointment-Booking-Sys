  ...
  return userData && (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      
      {
        ...
        ?<label htmlFor="image">
            <div className='inline-block relative cursor-pointer'>
              ...
              <img className='w-10 absolute botton-12 right-12' src={image ? "": assets.upload_icon} alt="" />
            </div>
      ...
      <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
        ...
        <p className='font-medium'>Address:</p>
        
           {
            ...
            ?
            <p>
              <input className='bg-gray-200' onChange={(e)=>setUserData(prev=>({...prev,address:{...prev.address,line1:e.target.value}}))}value={userData?.address?.line1} type="text" /><br />
      ...
      <div>
        ...
        isEdit 
        ?
        <select className='max-w-28 bg-gray-200' onChange={(e)=>setUserData(prev=>({...prev,gender:e.target.value}))} value={userData.gender}>
      ...
      <div className='mt-10'>
        {
          isEdit?
          <button className='border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white transition-all ' onClick={updateUserProfileData}>Save Info</button>
          :
